...
// Global Variables
string *currentMaze = nullptr; // Currently No String is Pointing
...
void startgame()
{
    ...
    { //              x, y
        ...
        printMaze();
        printScore();
        // _kbhit() khbit Stands for Keyboard Hit
        if (_kbhit())
        {

            char key = _getch(); // This Function will gets the pressed key
            if (key == -32)      // Special key prefix
            // As Arrow keys are special keys, So for Special Keys, -32 is a Spceial Key Prefix
            {
                key = _getch(); // Now we will Store Any Special key Pressed in a key Variable
                if (key == 75)  // Ascii value of Left Arrow Key is 75
                {
                    movePacmanLeft();
                }
                else if (key == 77)
            ...
            else if (key == 27) // ESC key Ascii Code is 27
            {
                gameRunning = false;     
            }
        }
        ...
        else if (score >= 3000 && level == 2) //You will win at Score = 3000
        {
            system("cls");
...
void ShowWinners()
{
    ...
    getch();
    fstream file2;
    file2.open("winners.txt", ios::in); // We will specify ios::in for reading

    if (!file2) // If file is not opened
    {
    ...
    else
    {
        cout << "==== WINNERS ====" << endl;
        string line;
        while (getline(file2, line)) // 'file2'
        {
            cout << line << endl;
        }
